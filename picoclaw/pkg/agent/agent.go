...
func (al *AgentLoop) runAgentLoop(
	...
	// Forward the channel-authenticated caller (e.g. WhatsApp phone) to MCP
	// tools so downstream servers can apply per-sender permissions.
	if raw := opts.Dispatch.InboundContext.Raw; raw != nil {
		if phone := raw["caller_phone"]; phone != "" {
			ctx = mcp.WithCaller(ctx, phone)
		}
	}
