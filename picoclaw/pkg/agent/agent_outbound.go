...
// customerFallbackMessage is shown to the person on WhatsApp when the agent
// can't produce a reply (e.g. all AI providers failed). It must never leak
// internal error details — the real error is logged separately.
const customerFallbackMessage = "Sorry, we're facing a brief technical issue and couldn't get to your message just now. 🙏 Please try again in a little while — we'll get back to you."

func (al *AgentLoop) maybePublishError(ctx context.Context, channel, chatID, sessionKey string, err error) bool {
	if errors.Is(err, context.Canceled) {
		return false
	}
	logger.WarnCF("agent", "Sending customer fallback after processing error", map[string]any{
		"channel": channel,
		"chat_id": chatID,
		"error":   err.Error(),
	})
	al.PublishResponseIfNeeded(ctx, channel, chatID, sessionKey, customerFallbackMessage)
	return true
}
