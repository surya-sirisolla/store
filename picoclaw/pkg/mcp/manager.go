...
// WithCaller attaches the channel-authenticated caller identity (e.g. a
// WhatsApp phone in +E.164) to ctx. CallTool forwards it to every MCP tool as
// a trusted "_caller" argument the model cannot set or spoof, so downstream
// servers can apply per-sender permissions.
...
func callerFromContext(ctx context.Context) string {
	c, _ := ctx.Value(callerCtxKey{}).(string)
	return c
}
...
func (m *Manager) CallTool(
	...
	// Forward the trusted caller identity (set by the channel) as a hidden
	// "_caller" argument, without mutating the caller's map.
	if caller := callerFromContext(ctx); caller != "" {
		merged := make(map[string]any, len(arguments)+1)
		for k, v := range arguments {
			merged[k] = v
		}
		merged["_caller"] = caller
		arguments = merged
	}
