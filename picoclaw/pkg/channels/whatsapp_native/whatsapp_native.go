...
// reportContactSeen tells the console backend that a number messaged the bot,
// so "who messaged the bot" is captured for every inbound message — even when
// the AI fails to reply or calls no tool. Fire-and-forget: it must never block
// or break message handling.
func reportContactSeen(phone, name, message string) {
	url := strings.TrimSpace(os.Getenv("CONTACT_INGEST_URL"))
	if url == "" {
		return
	}
	payload, _ := json.Marshal(map[string]string{"phone": phone, "name": name, "message": message})
	go func() {
...
// resolveCallerPhone returns the sender's phone number in +E.164 form, or ""
// if it can't be determined. WhatsApp may address the sender by LID (a privacy
...
func (c *WhatsAppNativeChannel) resolveCallerPhone(evt *events.Message) string {
	...
	case evt.Info.Sender.Server == types.DefaultUserServer:
		pn = evt.Info.Sender
	case evt.Info.SenderAlt.Server == types.DefaultUserServer:
		pn = evt.Info.SenderAlt
	case c.client != nil && evt.Info.Sender.Server == types.HiddenUserServer:
		if resolved, err := c.client.Store.LIDs.GetPNForLID(c.runCtx, evt.Info.Sender.ToNonAD()); err == nil {
			pn = resolved
		}
	}
...
func (c *WhatsAppNativeChannel) handleIncoming(evt *events.Message) {
	...
	// Recover the sender's real phone number (WhatsApp may address them by a
	// privacy LID) so downstream tools can apply per-sender permissions and so
	// the console can show who messaged — independent of whether the AI replies.
	if phone := c.resolveCallerPhone(evt); phone != "" {
		metadata["caller_phone"] = phone
		reportContactSeen(phone, evt.Info.PushName, content)
	}
